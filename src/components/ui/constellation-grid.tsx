...
export default function ConstellationGrid({
  ...
  return (
    <div
      ref={containerRef}
      className={`relative w-full h-screen overflow-hidden select-none bg-slate-950 dark:bg-slate-950 ${className}`}
    >
      <canvas ref={canvasRef} className="absolute inset-0 block cursor-crosshair z-0" />

      {/* Default Overlay if requested */}
      {showDefaultOverlay && (
      ...
      {/* Custom Child Elements (such as the requested Hero Typography) */}
      {children && (
